...
# --- 🔥 [MỚI] LOGIC ĐỒNG BỘ URL 🔥 ---

# Hàm cập nhật thanh địa chỉ
def update_query_param(chap_id):
    st.query_params["chap"] = str(chap_id)

# Khởi tạo Session State (Có check URL)
if 'current_chap_id' not in st.session_state:
    # 1. Kiểm tra xem trên URL có số chương không (?chap=...)
    query_params = st.query_params
    url_chap = query_params.get("chap", None)
    
    # 2. Nếu có và hợp lệ -> Dùng nó
    if url_chap and url_chap.isdigit() and int(url_chap) in chapter_ids:
        st.session_state['current_chap_id'] = int(url_chap)
    # 3. Nếu không -> Về chương 1
    else:
        st.session_state['current_chap_id'] = chapter_ids[0]
        update_query_param(chapter_ids[0]) # Set URL mặc định luôn
...
def local_css(font_family):
    ...
    <style>
        ...
        .content-text p {{
            font-family: {font_family}; font-size: var(--font-size);
...
# --- 5. HÀM HỖ TRỢ (CÓ CẬP NHẬT URL) ---
def go_to_chap(chap_id):
    st.session_state['current_chap_id'] = chap_id
    update_query_param(chap_id) # <--- Cập nhật URL ngay khi chuyển chương
...
def ai_rewrite(text):
    if not API_KEY: return "❌ Chưa có API Key"
    try:
...
if data:
    ...
    if not is_editor_mode:
        ...
        # Nav trên (Sửa để gọi go_to_chap -> update url)
        c1, c2, c3 = st.columns([1, 4, 1])
        if c1.button("⬅️"): 
            prev_id = chapter_ids[chapter_ids.index(current_id)-1] if chapter_ids.index(current_id)>0 else current_id
            go_to_chap(prev_id)
            st.rerun()
        if c3.button("➡️"): 
            next_id = chapter_ids[chapter_ids.index(current_id)+1] if chapter_ids.index(current_id)<len(chapter_ids)-1 else current_id
            go_to_chap(next_id)
            st.rerun()
